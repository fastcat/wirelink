...
func TestLocalFacts(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			[]*fact.Fact{
				testutils.EndpointFactFull(u1, &k1, expires),
			},
			false,
		},
		{
			...
			[]*fact.Fact{
				testutils.EndpointFactFull(u2, &k1, expires),
			},
			false,
		},
		{
			...
			[]*fact.Fact{
				testutils.AllowedIPFactFull(n1, &k1, expires),
				testutils.AllowedIPFactFull(n2, &k1, expires),
			},
			false,
		},
		{
