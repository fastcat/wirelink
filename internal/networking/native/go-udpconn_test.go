...
func TestGoUDPConn_ReadPackets(t *testing.T) {
	// synctest always starts at a fixed time. it's documented, but don't rely on
	// that out of sheer paranoia
	var tZero time.Time
	...
	tests := []struct {
		...
		},
		{
			...
			args{
				-1,
				[]*networking.UDPPacket{{Time: tZero, Data: packet1}},
				[]*networking.UDPPacket{{Time: tZero, Data: packet1}},
			},
			require.NoError,
			false, true,
		...
		},
		{
			...
			args{
				-1,
				[]*networking.UDPPacket{
					{Time: tZero, Data: packet1},
					{Time: tZero.Add(100 * time.Millisecond), Data: packet2},
				},
				[]*networking.UDPPacket{
					{Time: tZero, Data: packet1},
					{Time: tZero.Add(100 * time.Millisecond), Data: packet2},
			...
			synctest.Test(t, func(t *testing.T) {
				...
				randUDP := func() (*net.UDPAddr, networking.UDPConn) {
					for {
						...
						// need to enable polling mode for this to work under synctest
						udp, err := e.ListenUDPPoll("udp", addr)
				...
				go func() {
					...
					for i := range tt.args.send {
						packetOffset := tt.args.send[i].Time.Sub(tZero)
				...
				for i := 0; i < len(gotReceive) && i < len(tt.args.wantReceive); i++ {
					...
					wantOffset := want.Time.Sub(tZero)
					assert.GreaterOrEqual(t, gotOffset.Milliseconds(), wantOffset.Milliseconds(), "packet %d earliest receive time", i)
					assert.LessOrEqual(t, gotOffset.Milliseconds(), (wantOffset + 15*time.Millisecond).Milliseconds(), "packet %d latest receive time", i)
				}
			})
		})
	}
